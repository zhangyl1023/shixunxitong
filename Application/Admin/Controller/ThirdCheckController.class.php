...
class ThirdCheckController extends MyController
{
    public function showThirdCheck(){
        $model=new Model;
...
LEFT JOIN sx_sourse f on f.id=d.sourse_id where a.status=3 or a.status=4 or a.status=44");
...
//        $this->ajaxReturn($data);return;
    ...
    public function pass1(){
        ...
        $suoshu=I('get.suoshu');
        $model=M('');
        $data=$model->table("$suoshu")->where("id=$id")->setField("status","4");
        echo $data;
    }
    ...
    public function back1(){
        ...
        $suoshu=I('get.suoshu');
        $model=M('');
        $data=$model->table("$suoshu")->where("id=$id")->setField("status","44");
        echo $data;
    }
