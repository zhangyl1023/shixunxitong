...
class FirstCheckController extends MyController
{
    public function showFirstCheck(){
        $model=new Model;
...
LEFT JOIN sx_sourse f on f.id=d.sourse_id where a.status=2 or a.status=1 or a.status=22");
...
//        $this->ajaxReturn($data);return;
    ...
    public function cancelpass1(){
        ...
        $suoshu=I('get.suoshu');
        $model=M('');
        $data=$model->table("$suoshu")->where("id=$id")->setField("status","1");
        echo $data;
    }
    public function back1(){
        ...
        $suoshu=I('get.suoshu');
        $model=M('');
        $data=$model->table("$suoshu")->where("id=$id")->setField("status","22");
        echo $data;
    }
